...
- [ ] **`src/common/model_registry.py`**: `ModelRegistry` — two-tier index: (1) in-memory `dict[str, ArtifactRecord]` built at startup from YAML frontmatter of all `*.md` entity/connection files; `watchdog` filesystem listener keeps it incrementally fresh; `list_artifacts(**filter)` and `list_connections(**filter)` query this tier; (2) SQLite FTS5 table `artifact_fts(artifact_id UNINDEXED, content)` in `model_registry.db` — populated at startup and updated on every `write_artifact` call; `search(query, **filter)` queries FTS5 with `MATCH`, applies metadata filters, returns ranked `(ArtifactRecord, snippet)` pairs; optional sqlite-vec embedding tier (enabled when ≥50 artifacts and library available — same activation pattern as LearningStore); both tiers updated synchronously; thread-safe (`threading.RLock`)
