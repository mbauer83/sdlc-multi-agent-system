...
- **D3:** Load template via `read_framework_doc("framework/diagram-conventions.md §7.sequence")`. For each key interaction flow, author a sequence diagram. Keep each diagram scoped to a single causally coherent scenario; if flows are independently triggered, split them into separate sequence files and cross-reference them. Capture explicit feature relevance and cross-layer traceability: include `connection-ids-used` in frontmatter and state the related business process/service IDs in the diagram note or companion overview table. Message labels must be API-contract quality (verb-led operation names with explicit parameters, consistent command/query semantics by participant role), not prose. Use purpose-scoped filenames such as `architecture-repository/diagram-catalog/diagrams/runtime-sequence-<flow-id>-v1.puml` (phase token only when the sequence itself is phase-scoped) and write via `write_artifact`.
