...
| `artifact-type` | string | Yes | The artifact class **being produced** by this agent when the mistake occurred. One of the schema-id values from `framework/artifact-schemas/`: `architecture-vision`, `business-architecture`, `application-architecture`, `data-architecture`, `technology-architecture`, `implementation-plan`, `test-strategy`, `safety-constraint-overlay`, `architecture-contract`, `change-record`, `requirements-register`, `diagram-catalog`; or `process` for procedural/protocol mistakes (e.g., incorrect CQ routing, missed discovery scan step). **Rule:** Use the primary OUTPUT artifact of the skill, not the artifact reviewed/consumed as input. This ensures `query_learnings` retrieves the correction at the start of the next invocation of the same skill. Example: a CSCO learning about checking PII in AVs during Phase A gate review is tagged `safety-constraint-overlay` (CSCO's output), not `architecture-vision` (CSCO's input) — the correction text describes what to look for in the AV. |
