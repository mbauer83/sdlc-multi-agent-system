...
**Write-authority by layer:**
- `model-entities/motivation/`, `strategy/`, `business/`, `implementation/` — **SA** (Solution Architect)
- `model-entities/application/` — **SwA** (Software Architect / Principal Engineer); SA reads for traceability review
- `connections/` — **SA** for business-layer connections; **SwA** for application-layer connections (archimate/access, ER) and cross-layer connections
- `diagram-catalog/` — **SA** is sole writer (all agents submit `diagram.display-spec-request` handoffs to SA; SwA writes Phase C diagrams via SA integration)
- `decisions/` — **SA** for business-layer ADRs; **SwA** may contribute via handoff
- `overview/` — **SA** for AV/BA overviews; **SwA** for AA/DA overviews (aa-overview.md, da-overview.md)
- `change-records/` — **SA** for business-layer CRs; **SwA** for application/technology-layer CRs

```
architecture-repository/
  model-entities/        # All ArchiMate entity files — grouped by layer/aspect
    motivation/          # SA — STK, DRV, GOL, REQ, CST, PRI, MEA, VAL, ASS, OUT
