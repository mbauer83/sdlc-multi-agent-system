...
| 4.9 | ENG-001 reference model: 4.9a–e complete; 4.9f partial — 2/7 diagrams produced, semantically corrected, and fully connected (115 total connection files; all entities connected in phase-b); BCO-001 prefix correction (BusinessCollaboration); entity friendly-name filenames; 4.9g–h pending | Partial |
| 5 | Python implementation layer (EventStore completion + PydanticAI agents + LangGraph orchestration + source adapters); `pyproject.toml` + uv project setup complete; `src/common/model_verifier.py` complete (33 BDD tests passing, E104 entity filename check added, `entity_id_from_path()` utility); `src/common/archimate_types.py` canonical type registry complete | Partial |
