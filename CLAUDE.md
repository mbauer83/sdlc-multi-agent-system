...
| 4.9 | ENG-001 reference model: 4.9a–e complete (dir bootstrap, 25 motivation/strategy entities, 28 business-layer entities, 46 application-layer entities, 75 connection files, `_macros.puml` generated); 4.9f partial — 2/7 diagrams done (`phase-b-archimate-business-v1.puml`, `phase-c-archimate-application-v1.puml`); 4.9g–h (overview, ADRs, event-sourcing taxonomy) pending | Partial |
| 5 | Python implementation layer (EventStore completion + PydanticAI agents + LangGraph orchestration + source adapters); `pyproject.toml` + uv project setup complete; `src/common/model_verifier.py` complete (31 BDD tests passing) | Partial |
