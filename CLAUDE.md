...
22. **Every engagement work-repository mutation must be event-sourced into the engagement event-stream.** All §4.9h events (`artifact.created/updated`, `source.scanned`, `entity.confirmed`, `upload.registered`, `file.referenced`) are emitted by tool implementations (`ArtifactReadWriterPort.write()`, `scan_target_repo()`, `InteractionHandler`), not by skill instructions — `source_evidence` is auto-extracted from `[inferred: <source>]` annotations already in entity content; `entity.confirmed` is derived from `active_skill_id` context. No skill file changes are required for this invariant. Events reference file paths; file content lives in files. EventStore snapshots (`snapshots` table) are mandatory for bounded startup time — situative triggers at `engagement.started`, `gate.evaluated/passed`, `sprint.close`, and periodic every 100 events. Full taxonomy and snapshot spec in `specs/IMPLEMENTATION_PLAN.md §4.9h` and §5a.
