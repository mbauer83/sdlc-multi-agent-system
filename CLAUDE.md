...
| 5 | Python implementation layer (EventStore completion + PydanticAI agents + LangGraph orchestration + source adapters); `pyproject.toml` + uv project setup complete; `src/common/` fully implemented; MCP servers (model + registry + framework) complete; `src/events/replay.py` + `replay_builder.py` (full 20-event state machine); `src/agents/` MVP: `deps.py`, `base.py` (build_agent Layer 1+2+3), `skill_loader.py` (budget-enforced extraction), `tools/` (universal + escalation + learning + write tools); `src/orchestration/` MVP: `graph_state.py` (SDLCGraphState), `session.py` (EngagementSession + CLI `python -m src.orchestration.session`); `scripts/demo_setup.py` + `demo_teardown.py` (scenario-based integration test: TaskFlow API Phase A with 7-check verification); pending: `src/agents/memento_store.py`, per-role agent modules, LangGraph PM+phase subgraph wiring (Stage 5c), source adapters (Stage 5d) | Partial |
| 5.5 | Engagement dashboard (local web server + PUML rendering + filesystem monitoring) | Pending |
| 5.6 | LLM provider configuration: `LLMConfig` (PydanticAI `provider:model-id` format), provider SDK optional extras in pyproject.toml, `DOB-015` entity, model/doc corrections to remove hardcoded Anthropic model strings. PydanticAI and LangGraph are permanent commitments — not abstracted. **5.6-A complete** (LLMConfig model + cascade load, base.py + session.py updated, pyproject.toml extras, engagements-config llm block, demo --model flag). **5.6-B pending** (AIF-002/CST-001/APP entities, DOB-015, doc corrections). | Partial |
