...
14. **Every AGENT.md and every skill file must have YAML frontmatter.** Frontmatter provides machine-readable routing metadata for the Python orchestration layer. AGENT.md required fields: `agent-id`, `name`, `display-name`, `role-type`, `vsm-position`, `primary-phases`, `invoke-when`, `owns-repository`, `personality-ref`, `skill-index`, `runtime-ref`, `system-prompt-identity`. The `system-prompt-identity` field is the compact static Layer 1 system prompt used by `build_agent()` in `src/agents/base.py` — 3–5 sentences covering: role name/abbreviation, authority domain, owned repository, one non-negotiable constraint. Skill file required fields: `skill-id`, `agent`, `name`, `invoke-when`, `trigger-phases`, `trigger-conditions` (LLM-hint prose; not machine-parsed), `entry-points`, `primary-outputs`, `complexity-class` (`simple | standard | complex` — governs SkillLoader token budget: simple ≤700/840, standard ≤1400/1680, complex ≤2350/2820). The `trigger-phases` list is the machine-readable routing key; `trigger-conditions` is documentation only. **`invoke-when` and `invoke-never-when` must be written in plain English with no unexplained abbreviations** — these fields are read by LLMs for autonomous skill selection (PM agent via registry tool calls, Express Mode without a `--skill` flag, any external use of the skill file) and have no guaranteed pre-injected glossary. Write them as self-contained sentences a reader unfamiliar with this framework can understand. (`trigger-conditions` and `trigger-phases` are documentation/code-only and are not injected to any LLM.) **Runtime extraction contract:** only two AGENT.md extractions reach the live agent — `system-prompt-identity` (Layer 1, always) and `### Runtime Behavioral Stance` from §11 (Layer 2, always). All other AGENT.md content is authoring documentation that governs skill file authoring — it does not reach the agent directly. Skill sections are assembled by `SkillLoader` in `assembly_position` order per the `SectionSpec` registry in `src/agents/_skill_sections.py`. All nine known section types (Inputs Required, Procedure/Steps, Common Rationalizations, Feedback Loop, Red Flags, Algedonic Triggers, Verification, Outputs, End-of-Skill Memory Close) are registered with their `modes` (workflow/express), `assembly_position`, `truncation_priority`, and `truncation_strategy`. Sections not in the registry must carry a `<!-- workflow -->` or `<!-- express -->` heading comment or they will be silently excluded from agent context. `AgentDeps.invocation_mode` (`"workflow"` | `"express"`, default `"workflow"`) controls which sections are included in Layer 3. Use `framework/agent-index.md` as the compact routing reference (~500 tokens). Governed by `framework/agent-runtime-spec.md`.
...
| 5 | Python implementation layer. All core components complete: EventStore+Alembic, replay, agent base+skills+tools (universal/escalation/learning/write/target_repo/memento/diagram), LangGraph graph+routing+nodes, EngagementSession, TargetRepoManager+BDD, MementoStore+BDD. **diagram_tools complete:** 4 agent-facing tools + `regenerate_macros` auto-triggered by all write paths. **222 BDD tests.** **Live integration test 6/7 pass** (7th pending artifact-type fix re-run). **Memento wiring gaps documented** (invocation_id bug, phase not in AgentDeps, no MementoStore sharing, no memento.saved event, no BDD tests, express no-op unguarded) — `stage-5b-memento-wiring` checklist in IMPLEMENTATION_PLAN.md. Pending: PM LLM routing loop, external source adapters, skill condensation, SwA Phase C demo step. | Partial |
| 5.5 | Engagement dashboard (local web server + PUML rendering + filesystem monitoring) | Pending |
| 5.6 | LLM provider configuration: `LLMConfig` (PydanticAI `provider:model-id` format), provider SDK optional extras in pyproject.toml, `DOB-015` entity, model/doc corrections to remove hardcoded Anthropic model strings. PydanticAI and LangGraph are permanent commitments — not abstracted. **5.6-A+B complete** (LLMConfig model + cascade load, base.py + session.py updated, pyproject.toml extras, engagements-config llm block, demo --model flag; AIF-002/CST-001/APP-005/007/008/009 entity corrections; DOB-015 + serving connection; CLAUDE.md + coding guidelines updated). | Partial |
| 5.7 | Skill quality hardening: SectionSpec registry (`_skill_sections.py`) replaces `_INCLUDED_H2`; mode-aware filtering (`workflow`/`express`); budget raised (complex 2350/2820); `<!-- workflow -->` tags on all 48 skill files; `invoke-never-when` stubs + `## Common Rationalizations`/`## Red Flags`/`## Verification` stubs added to all skills via `scripts/patch_skill_sections.py`. TODO-marker content fill-in pending. E.3/E.4 (boilerplate extraction, AGENT.md preamble, abbreviation definitions) pending. | Partial |
| 5.8 | Express Mode: standalone skill invocation without an engagement session. `src/express.py` (`run_express_skill()` API) + `src/cli.py` `express` subcommand; mode-filtered skill loading via SectionSpec registry; Layer 1 behavioral override for no-engagement context; `## Express Mode` sections in 5 candidate skills (SA-REV-A, SWA-REV-C, SA-PHASE-A, CSCO-STAMP-STPA, DE-PHASE-G). Depends on Stage 5.7-E. | Pending |
