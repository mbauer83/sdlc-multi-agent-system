...
def test_incremental_state_invalidates_when_engine_signature_changes(
    tmp_path: Path,
    monkeypatch,
) -> None:
    repo = _make_repo(tmp_path)
    ...
    state_files = list(state_dir.glob("*.state-v1.json"))
    assert state_files, "incremental mode should persist state"
    state_path = state_files[0]
    cached = json.loads(state_path.read_text(encoding="utf-8"))

    entity_path = repo / "model-entities" / "application" / "components" / "APP-001.md"
    stat_before = entity_path.stat()
    entity_path.write_text(
        VALID_ENTITY.replace("status: draft", "status: wrong"),
        encoding="utf-8",
    )
    # Keep mtime unchanged so file inventory diff alone would miss this edit.
    os.utime(entity_path, ns=(stat_before.st_atime_ns, stat_before.st_mtime_ns))

    # Corrupt state metadata to emulate stale cache from an older verifier engine.
    cached["engine_signature"] = "stale-engine-sign"
    state_path.write_text(json.dumps(cached, separators=(",", ":"), ensure_ascii=True), encoding="utf-8")
