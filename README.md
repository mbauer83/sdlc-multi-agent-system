...
| `src/agents/` MVP — `deps.py` (AgentDeps), `base.py` (build_agent: Layer 1+2 from AGENT.md frontmatter + Layer 3 via SkillLoader), `skill_loader.py` (budget-enforced section extraction, truncation priority, budget warnings); `tools/`: `universal_tools.py`, `escalation_tools.py`, `learning_tools.py`, `write_tools.py` (path-boundary enforcement + ALG-007 signalling) | Complete |
...
| `src/agents/` remaining — per-role agent modules (`solution_architect.py`, `project_manager.py`, etc.), `memento_store.py` (MementoStore wrapping LangGraph BaseStore), `get_memento_state()`/`save_memento_state()` tools | Pending |
| `src/orchestration/` Stage 5c — `nodes.py`, `routing.py`, `graph.py` (full LangGraph PM supervisor + phase subgraphs), `promotion.py`, `user_interaction.py` | Pending |
| `src/sources/` Stage 5d — `target_repo.py` (TargetRepoManager + worktree), external source adapters (Confluence, Jira, Git) | Pending |
