...
| Agent and skill files (all `agents/<role>/`) — Stage 4.9j retroactive: Step 0.M (Memento Recall) + End-of-Skill Memory Close added to all 48 skill files via `scripts/add_step0m_and_memory_close.py` | Complete |
...
| `src/events/replay.py` + `replay_builder.py` — full 20-event state machine (cycle, phase, sprint, gate, artifact, CQ, algedonic); mutable `StateBuilder` builder pattern; audit-only events pass through; forward-compatible unknown event skip | Complete |
| `src/agents/` MVP — `deps.py` (AgentDeps + `invocation_mode: Literal["workflow","express"]`), `base.py` (build_agent: Layer 1+2+3), `skill_loader.py` + `_skill_sections.py` (SectionSpec registry; mode-aware workflow/express filtering; data-driven assembly+truncation; budget raised simple 700/840, standard 1400/1680, complex 2350/2820; `invoke_when`/`invoke_never_when` on SkillSpec); `tools/`: `universal_tools.py`, `escalation_tools.py`, `learning_tools.py`, `write_tools.py`, `diagram_tools.py` + `_diagram_io.py` (4 SA/SwA diagram tools) | Complete |
| `src/orchestration/` MVP — `graph_state.py` (SDLCGraphState TypedDict with multi-repo fields + LangGraph add_messages), `session.py` (EngagementSession: boot + EventStore + single-agent invoke); CLI: `python -m src.orchestration.session`; SDLC_MODEL env var for model override; full stack smoke test passing | Complete |
| `scripts/demo_setup.py` — scenario-based integration test CLI: scaffolds ENG-DEMO engagement + TaskFlow API target repo, invokes SA agent (SA-PHASE-A), runs 7-check verification suite (vision doc, entity counts, algedonic log, EventStore events, ModelVerifier). `scripts/demo_teardown.py` — removes ENG-DEMO and reverts config. Supporting: `_demo_taskflow.py` (scenario assets), `demo_scaffold.py` (engagement scaffolding), `demo_verify.py` (verification). Usage: `uv run python scripts/demo_setup.py [--verbose] [--skip-run]` | Complete |
| `src/agents/roles.py` — per-role builder functions for all 9 roles; SA + SwA get `register_diagram_tools()` | Complete |
| `src/orchestration/` Stage 5c — `nodes.py`, `routing.py`, `graph.py` (full LangGraph PM supervisor + phase subgraphs), `promotion.py`, `user_interaction.py` | Partial |
...
| Stage 5.7 — Skill quality hardening: SectionSpec registry implemented; `<!-- workflow -->` tags on all 48 skill files; `invoke-never-when` + `## Red Flags` + `## Verification` stubs on all 42 complex+standard skills; `## Common Rationalizations` stubs on all 18 complex skills; `scripts/patch_skill_sections.py` (dry-run + bulk apply). TODO-content fill-in + E.3/E.4 boilerplate extraction pending. | Partial |
